...
const routes = [
    { path: 'customers', loadChildren: () => Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./customers/customer.module */ "./src/app/customers/customer.module.ts")).then(mod => mod.CustomerModule) },
    { path: 'orders', loadChildren: () => Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./orders/order.module */ "./src/app/orders/order.module.ts")).then(mod => mod.OrdersModule) },
...
let CustomerNewComponent = class CustomerNewComponent {
    constructor(service) {
        this.service = service;
...
CustomerNewComponent.ctorParameters = () => [
...
let CustomerListComponent = class CustomerListComponent {
    ...
    ngOnInit() {
        ...
        this.service.getAllCustomer().subscribe((customers) => {
...
let FiliterTextComponent = class FiliterTextComponent {
    constructor() {
        this._filiter = "";
...
let CustomerTableComponent = class CustomerTableComponent {
    ...
    loadSavedList() {
        if (localStorage.getItem(this.keyword)) {
            const strCachedDate = localStorage.getItem(this.keyword);
    ...
    calcualteOrderTotal() {
        ...
        if (this.filtertedCustomers) {
            this.filtertedCustomers.forEach((cust) => {
                this.customersOrderTotal += cust.orderTotal;
            });
        }
    }
    sortCustomers(columnName) {
        if (this.isAscendingSort)
            this.filtertedCustomers.sort((a, b) => a[columnName] > b[columnName] ? 1 : -1);
        else
            this.filtertedCustomers.sort((a, b) => a[columnName] < b[columnName] ? 1 : -1);
    ...
    applyFilter(data) {
        if (data) {
            this.filtertedCustomers = this.filtertedCustomers.filter((cust) => {
...
let OrderNewComponent = class OrderNewComponent {
    constructor(service, route, router, formBuilder, location) {
        this.service = service;
        this.route = route;
        this.router = router;
...
let AuthorizeGuard = class AuthorizeGuard {
    ...
    canActivate(route, state) {
        if (localStorage.getItem(src_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].userInfo)) {
            let userInfo = JSON.parse(localStorage.getItem(src_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].userInfo));
...
let NameFormatterPipe = class NameFormatterPipe {
    transform(value) {
        let fullName;
        if (value) {
            let nameSplit = value.toString().split(' ');
            let firstName;
            let lastName;
...
class Order {
    constructor(orderId, customerId, product, orderOty, unitPrice) {
